...
test('應用程式匯入並建立螢幕方向與觸控作答整合', async () => {
  ...
  assert.match(source, /import \{ createBattleOrientationController \} from '\.\/battle-orientation\.mjs';/);
...
test('對戰 timer 只會在橫向、計時且未結束時建立一次', async () => {
  ...
  assert.match(timer, /orientationPaused/);
...
test('直向暫停與橫向恢復依序管理輸入、CPU、timer 與畫面', async () => {
  ...
  assert.match(orientation, /if \(portrait\)[\s\S]*orientationPaused[\s\S]*orientationPaused = true;[\s\S]*battleInputGate\.disable\(\);[\s\S]*cpuController\.pause\(\);[\s\S]*clearBattleTimer\(\);[\s\S]*renderGame\(\);/);
  assert.match(orientation, /if \(!orientationPaused[\s\S]*!hasLiveBattle\(\)\) return false;[\s\S]*orientationPaused = false;[\s\S]*renderGame\(\);[\s\S]*cpuController\.resume\(\);[\s\S]*battleInputGate\.enable\(\);[\s\S]*startBattleTimer\(\);/);
});
...
test('直向時不排程 CPU 作答', async () => {
  ...
  assert.match(scheduler, /if \(orientationPaused\) return false;/);
...
test('對戰結果、題庫、主選單、開局失敗與停止活動都會清理方向狀態', async () => {
  ...
  assert.match(cleanup, /orientationController\.exitBattle\(\);[\s\S]*orientationPaused = false;/);
